from django.views.generic import CreateView, ListView
from django.urls import reverse_lazy
from orders.models import Customer, Product, Order
from orders.forms import CustomerCreateForm, ProductCreateForm, OrderCreateForm


class CustomerCreateView(CreateView):
    template_name = 'orders/customer-create/customer-create.html'
    model = Customer
    form_class = CustomerCreateForm
    success_url = reverse_lazy('home-template')


class CustomerListView(ListView):
    template_name = 'orders/customer-list/customer-list.html'
    model = Customer
    context_object_name = 'customers'


class ProductCreateView(CreateView):
    template_name = 'orders/product-create/product-create.html'
    model = Product
    form_class = ProductCreateForm
    success_url = reverse_lazy('home-template')


class OrderCreateView(CreateView):
    template_name = 'orders/order-create/order-create.html'
    model = Order
    form_class = OrderCreateForm
    success_url = reverse_lazy('home-template')
